...
describe('Token', () => {
  ...
  describe('#equals', () => {
    ...
    it('false if chain id differs', () => {
      expect(new Token(ChainId.MOONBASE, ADDRESS_ONE, 18).equals(new Token(ChainId.STANDALONE, ADDRESS_ONE, 18))).toBe(
        false
      )
    })
    ...
    it('true on reference equality', () => {
      const token = new Token(ChainId.STANDALONE, ADDRESS_ONE, 18)
      expect(token.equals(token)).toBe(true)
    })

    it('true even if name/symbol/decimals differ', () => {
      const tokenA = new Token(ChainId.STANDALONE, ADDRESS_ONE, 9, 'abc', 'def')
      const tokenB = new Token(ChainId.STANDALONE, ADDRESS_ONE, 18, 'ghi', 'jkl')
